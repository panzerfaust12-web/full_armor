...
func _ready() -> void:
	...
	target_position.y = -(rest_dist + wheel_radius + wheel_radius + over_extend)
...
func apply_wheel_physics(tank) -> void:
	...
	var suspension_force = (spring_force - spring_damp_f)
	...
	var steering_x_speed = global_basis.x.dot(tire_vel)
	grip_factor = absf(steering_x_speed/tire_vel.length())
	
	var x_traction = grip_factor #grip_curve.sample_baked(grip_factor)
	...
	var x_force = -global_basis.x * steering_x_speed * x_traction * ((tank.mass * gravity)/tank.wheels_total)
	
	## Tire Z Traction
	var f_speed = forward_dir.dot(tire_vel)
	var z_friction = z_traction
	var z_force = global_basis.z * f_speed * z_friction * ((tank.mass * gravity)/tank.wheels_total)
	
	## Counter Ramp Slide
	if absf(f_speed) < 0.1:
		var susp = global_basis.y.normalized() * suspension_force
		z_force.z -= susp.z * tank.global_basis.y.dot(Vector3.UP)
		x_force.x -= susp.x * tank.global_basis.y.dot(Vector3.UP)
